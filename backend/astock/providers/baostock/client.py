...
class BaostockQueryError(Exception):
    """baostock 登录或查询失败。"""
...
def query_error(label: str, rs) -> str | None:
    if rs.error_code != "0":
...
def safe_baostock_call[T](
    ...
    *,
    ...
    """执行 fn；超时/连接异常时抛 BaostockQueryError。"""
    try:
        return fn()
    except (socket.timeout, OSError, BaostockRecvTimeoutError) as e:
        msg = f"{label}: {e}"
        getattr(logger, log_level)(msg)
        raise BaostockQueryError(msg) from e
