...
BULL_MARKETS = {
    ...
    '2025年牛市': {'start': '2024-07-01', 'end': '2026-1-31'}
}
...
def print_bull_market_stats(bull_markets: dict, mode: str, threshold: float):
    """通用牛市统计结果打印"""
    if mode == 'point':
...
def print_top_turnover(df: pd.DataFrame, top_n: int = 5):
    """打印成交额Top5：分别统计沪深合计和三市合计"""
    if df.empty:
        print('\n未找到成交额记录\n')
        return
    ...
    top_all = df.sort_values('turnover', ascending=False).head(top_n)
    ...
    print(f'\n成交额Top{top_n}（上证+深证+创业板）：')
    for _, row in top_all.iterrows():
        date_str = row["date"].strftime("%Y-%m-%d")
        sh = format_amount(row["sh_amount"])
        sz = format_amount(row["sz_amount"])
        cyb = format_amount(row["cyb_amount"])
        total = format_amount(row["turnover"])
        print(f'  {date_str}: 上证{sh} + 深证{sz} + 创业板{cyb} = {total}')
    
    print()


def run_point_mode(threshold: float):
    """点位模式"""
...
def run_turnover_mode(threshold: float):
    """成交额模式"""
