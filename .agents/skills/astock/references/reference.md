...
| 数据源 | `backend/astock/sources/{baostock,akshare,tencent,market_overview}/` |
...
实现于 `services/imports/stock/`（`context` / `snapshot` / `path_select` / `history` / `importer`）：

1. 以 turnover 表最新日期为 `as_of_date`
2. akshare 全市场快照（失败则 baostock 代码池 + 腾讯批量）→ 过滤 `market_cap > MARKET_CAP_THRESHOLD`
3. Path A：结算日快照直写；Path B：ProcessPool（默认 8 worker）并发拉个股历史
4. 仅保留 `amount >= STOCK_TURNOVER_SLICE_THRESHOLD`（300 亿/日）切片
5. 缓冲 flush upsert；SSE 上报进度
