...
│   │   ├── sources/           # 外部数据源（baostock/、akshare/、tencent/、market_overview/）
...
| 层 | 职责 | 约定 |
|----|------|------|
| `sources/` | 单一外部数据源拉取，返回 `SourceFetchResult` | 不写库、不做业务计算、不依赖 services；按数据域分包（`baostock/`、`akshare/`、`tencent/`、`market_overview/`） |
...
| 全球市场概览 | `services/market_overview/` + `market_overview.yaml` | 6 类 13 项实时概览 |
...
- **症状：全球资产/概览数据 stale 或不刷新**
  ...
  - 再看：`services/global_asset/` / `services/market_overview/` 失败标记冷却
- **症状：改了前端但线上没变化**
